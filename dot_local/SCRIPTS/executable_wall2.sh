...
STATIC_WALLPAPER_DIR="$HOME/wallpapers/walls" 
# Live wallpaper directory (videos/gifs)
LIVE_WALLPAPER_DIR="$HOME/wallpapers/live-walls"
...
if [ -f "$selected_path" ]; then
    ...
    if [ "$type" = "static" ]; then
        ...
        magick "$selected_path"[0] -strip \
            \( -clone 0 -fill black -colorize 100 -alpha set -channel A -evaluate set 50% \) \
            -compose over -composite "$HOME/.cache/wall.dark"
        ...
        magick "$HOME/.cache/wall" -strip -scale 10% -blur 0x3 -resize 100% "$HOME/.cache/wall.blur"
        magick "$selected_path"[0] -strip \
            \( -clone 0 -fill black -colorize 100 -alpha set -channel A -evaluate set 50% \) \
            -compose over -composite "$HOME/.cache/wall.dark"
        magick "$HOME/.cache/wall.sqre" \
