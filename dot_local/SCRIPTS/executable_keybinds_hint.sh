...
# r_override="window {$r_height $r_width \; border-radius: ${wind_border}px;} entry {border-radius: ${elem_border}px;} element {border-radius: ${elem_border}px;} ${r_listview} "
...
selected=$(echo -e "$output" | rofi -dmenu -i \
        -p "" \
        -display-columns 1 \
        -display-column-separator ":::" \
        -markup-rows \
        -theme ~/.config/rofi/keyhint.rasi)
