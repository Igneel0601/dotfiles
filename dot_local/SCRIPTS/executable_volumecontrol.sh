...
isVolumeBoost="${VOLUME_BOOST:-true}"
# Define functions
...
while getopts "iop:stq" opt; do
    ...
    p)
        ...
        ;;
    s)
        mapfile -t available_outputs < <(select_output)
        if ! selected_output=$(hyprland-dialog \
            --title "Choose an output device" \
            --buttons "$(printf "%s;" "${available_outputs[@]}")"); then
            selected_output=$(printf "%s\n" "${available_outputs[@]}" | rofi -dmenu -p "Output Device")
        fi
        [ -n "$selected_output" ] && select_output "$selected_output"
        exit
        ;;
    t)
        toggle_output
